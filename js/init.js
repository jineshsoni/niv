...
function constructify_tm_switch(){
	
	"use strict";
	
	var switchOpener		= jQuery('.constructify_tm_switch a');
	var switchContent		= jQuery('.buildify_tm_switch_content');
	var button				= jQuery('.buildify_tm_switch_content .inner.border input');
	var wrapper				= jQuery('.constructify_tm_wrapper_all');
	var button2				= jQuery('.buildify_tm_switch_content .inner.helpful input');
	var borderButton		= jQuery('.buildify_tm_switch_content .border-color a');
	var borderDivs			= jQuery('.constructify_tm_left_border,.constructify_tm_right_border,.constructify_tm_top_border,.constructify_tm_bottom_border');
	
	button.on('click',function(){
		
		if(wrapper.hasClass('fixed_border')){
			wrapper.attr('data-border','none');
			wrapper.removeClass('fixed_border');
		}else{
			wrapper.attr('data-border','fixed');
			wrapper.addClass('fixed_border');
		}
		jQuery('.constructify_tm_hero_header_wrap .owl-carousel').owlCarousel().trigger('refresh.owl.carousel');
		constructify_tm_stickyy();
		jQuery('.jarallax').each(function(){
			var element			= jQuery(this);
			var	customSpeed		= element.data('speed');
	...
	button2.on('click',function(){
		...
		if(wrapper.hasClass('helpful')){
			wrapper.attr('data-short-info','yes');
			wrapper.removeClass('helpful');
		}else{
			wrapper.attr('data-short-info','none');
			wrapper.addClass('helpful');
		}
		jQuery('.constructify_tm_hero_header_wrap .owl-carousel').owlCarousel().trigger('refresh.owl.carousel');
		constructify_tm_stickyy();
		jQuery('.jarallax').each(function(){
	...
	borderButton.on('click',function(){
		...
		var element			= jQuery(this)	;
		var data			= element.attr('data-color');
		borderButton.removeClass('opened');
		borderDivs.css({backgroundColor: data});
		element.addClass('opened');
		return false;
	});
	
	switchOpener.on('click',function(){
		
		
		if(switchOpener.hasClass('opened')){
			switchContent.removeClass('opened');
			switchOpener.removeClass('opened');
		}else{
			switchContent.addClass('opened');
			switchOpener.addClass('opened');
		}
